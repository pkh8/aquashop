...
//	public Long addOrder(OrderItemDto orderItemDto, String email) {
//		Item item = itemRepository.findById(orderItemDto.getItemId()).orElseThrow(EntityNotFoundException::new); // 결제할 상품 엔티티를 조회
//		Member member = memberRepository.findByEmail(email);    // 현재 로그인한 회원 엔티티를 조회
//
//		Order order = orderRepository.findByMemberId(member.getId()); // 현재 로그인한 회원의 주문 엔티티를 조회
//
//
//	}
